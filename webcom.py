...
for j in range(6):
    try:
        source = requests.get("https://www.amazon.com/best-sellers-books-Amazon/zgbs/books/ref=zg_bs_pg_"+str(j)+"?ie=UTF8&pg="+str(j))
        ...
        for i in range(l):
            ...
            try:
                url="https://www.amazon.com"+(mainfray[i].find('a', class_="a-link-normal")['href']).encode('utf-8')
            except:
                url="Not Available"
            try:
